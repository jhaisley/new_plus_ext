...
suite('Configuration Management Integration Tests', () => {
  ...
  test('Should use VS Code workspace settings', async () => {
    ...
    vscode.workspace.getConfiguration = (section?: string) => {
      const mockConfig = {
        get: (key: string) => {
          if (key === 'templatesPath') return 'C:\\Custom\\Templates';
          if (key === 'showQuickPick') return false;
          if (key === 'createSubfolders') return true;
          if (key === 'variables') return { customVar: 'customValue' };
          return undefined;
        },
        has: (key: string) => true,
  ...
  test('Should expand environment variables in templates path', async () => {
    ...
    vscode.workspace.getConfiguration = () => ({
      get: (key: string) => {
        if (key === 'templatesPath') return '%LOCALAPPDATA%\\VSCode\\Templates';
        return undefined;
      },
      has: () => true,
      inspect: () => ({}),
  ...
  test('Should handle relative paths relative to workspace', async () => {
    ...
    vscode.workspace.getConfiguration = () => ({
      get: (key: string) => {
        if (key === 'templatesPath') return './templates';
        return undefined;
      },
      has: () => true,
      inspect: () => ({}),
  ...
  test('Should validate configuration values', async () => {
    ...
    vscode.workspace.getConfiguration = () => ({
      get: (key: string) => {
        if (key === 'templatesPath') return ''; // Invalid empty path
        if (key === 'variables') return 'invalid'; // Invalid variables type
        return undefined;
      },
      has: () => true,
      inspect: () => ({}),
  ...
  test('Should handle configuration reload on settings change', async () => {
    let configurationChangeListener: ((e: vscode.ConfigurationChangeEvent) => void) | undefined;
    ...
    try {
      ...
      if (configurationChangeListener) {
        const mockChangeEvent: vscode.ConfigurationChangeEvent = {
  ...
  test('Should provide configuration schema validation', async () => {
    ...
    for (const invalidConfig of invalidConfigs) {
      const isValid = configService.validateConfiguration(invalidConfig as any);
  ...
  test('Should handle configuration migration from older versions', async () => {
    ...
    vscode.workspace.getConfiguration = () => ({
      get: (key: string) => {
        ...
        if (key === 'templateDir') return 'C:\\Old\\Templates'; // Old key name
        if (key === 'enableQuickPick') return true; // Old key name
        return undefined;
      },
      has: (key: string) => key === 'templateDir' || key === 'enableQuickPick',
      inspect: () => ({}),
      update: async () => {}
    } as any);

...
import { ValidationUtils, FileOperationUtils } from '../../src/utils';
...
suite('Error Handling and Edge Cases Tests', () => {
  ...
  suite('Template Service Error Handling', () => {
    ...
    test('Should handle corrupted template.json files', async () => {
      ...
      const originalReadFile = FileOperationUtils.readFile;
      FileOperationUtils.readFile = async (filePath: string) => {
      ...
      try {
        ...
        FileOperationUtils.readFile = originalReadFile;
      }
    });
    ...
    test('Should handle missing required template.json properties', async () => {
      ...
      const isValid = templateService.validateTemplate(invalidTemplate as any);
    ...
    test('Should handle templates with invalid type values', async () => {
      ...
      for (const template of invalidTemplates) {
        const isValid = templateService.validateTemplate(template as any);
    ...
    test('Should handle circular reference in template discovery', async () => {
      ...
      const originalDirectoryExists = FileOperationUtils.directoryExists;
      const originalListDirectory = FileOperationUtils.listDirectory;
      
      let visitedPaths = new Set<string>();
      
      FileOperationUtils.directoryExists = async (path: string) => true;
      FileOperationUtils.listDirectory = async (path: string) => {
      ...
      try {
        ...
        FileOperationUtils.directoryExists = originalDirectoryExists;
        FileOperationUtils.listDirectory = originalListDirectory;
      }
    });
  });
  ...
  suite('File Operation Error Handling', () => {
    test('Should handle permission denied errors', async () => {
      ...
      const originalWriteFile = FileOperationUtils.writeFile;
      FileOperationUtils.writeFile = async (filePath: string, content: string) => {
      ...
      try {
        ...
        FileOperationUtils.writeFile = originalWriteFile;
      }
    });
    ...
    test('Should handle network drive unavailable', async () => {
      ...
      FileOperationUtils.writeFile = async (filePath: string, content: string) => {
        if (filePath === networkPath) {
          const error = new Error('ENETUNREACH: network is unreachable') as any;
    ...
    test('Should handle extremely long file paths', async () => {
      ...
      const isValid = ValidationUtils.isValidPath(longPath);
    ...
    test('Should handle file names with invalid characters', async () => {
      ...
      for (const name of invalidNames) {
        const isValid = ValidationUtils.isValidFileName(name);
  ...
  suite('Command Execution Error Handling', () => {
    ...
    test('Should handle concurrent command execution', async () => {
      ...
      const promises = [];
      for (let i = 0; i < 5; i++) {
        promises.push(newFromTemplateCommand.createFromTemplate(
          template,
          '/target',
          `test${i}.txt`,
          new Map()
  ...
  suite('Edge Cases and Boundary Conditions', () => {
    ...
    test('Should handle Unicode and special character content', async () => {
      const unicodeTemplate: Template = {
        name: 'Unicode',
        type: 'file',
        path: '/test',
        files: [{
    ...
    test('Should handle maximum path length variations across platforms', async () => {
      ...
      for (const path of testPaths) {
        const isValid = ValidationUtils.isValidPath(path);
    ...
    test('Should handle zero-byte files', async () => {
      const zeroBytesTemplate: Template = {
        name: 'ZeroBytes',
        type: 'file',
        path: '/test',

...
suite('Performance Validation Tests', () => {
    ...
    suite('Template Creation Performance', () => {
        test('Should create simple file template in under 500ms', async () => {
            ...
            const startTime = Date.now();
            const result = await newFromTemplateCommand.createFromTemplate(simpleTemplate, '/target', 'simple.txt', new Map());
            const endTime = Date.now();
            const duration = endTime - startTime;
        ...
        test('Should create folder template with 50 files in under 5 seconds', async () => {
            ...
            const startTime = Date.now();
            const result = await newFromTemplateCommand.createFromTemplate(folderTemplate, '/target', 'medium-project', new Map());
            const endTime = Date.now();
            const duration = endTime - startTime;
            assert.ok(result.success);
        ...
        test('Should handle large file content efficiently', async () => {
            ...
            const initialMemory = process.memoryUsage();
            const startTime = Date.now();
            const result = await newFromTemplateCommand.createFromTemplate(largeFileTemplate, '/target', 'large.txt', new Map());
            const endTime = Date.now();
            const finalMemory = process.memoryUsage();
            const duration = endTime - startTime;
        ...
        test('Should batch file operations for better performance', async () => {
            ...
            newFromTemplateCommand.createFile = async (...args) => {
                fileOperationCount++;
                return originalCreateFile.apply(newFromTemplateCommand, args);
            };
            const startTime = Date.now();
            const result = await newFromTemplateCommand.createFromTemplate(batchTemplate, '/target', 'batch-project', new Map());
            const endTime = Date.now();
            const duration = endTime - startTime;
    ...
    suite('Variable Processing Performance', () => {
        ...
        test('Should cache variable processing results', async () => {
            ...
            variableService.processTemplate = (tmpl, vars) => {
                processingCallCount++;
    ...
    suite('Concurrent Operations', () => {
        test('Should handle concurrent template creation without conflicts', async () => {
            ...
            const promises = Array.from({ length: 10 }, (_, i) => newFromTemplateCommand.createFromTemplate(template, '/target', `concurrent${i}.txt`, new Map()));
            const results = await Promise.all(promises);
            const endTime = Date.now();
            const duration = endTime - startTime;
            // All should succeed
    ...
    function createMockTemplates(count) {
